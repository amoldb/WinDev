...
LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static int cyClient, iVscollPos = 0;
	HDC hdc = NULL;
	FILE* fp = NULL;
	int iCnt=0;
	char chBuffer[bSize];
		...
		case WM_VSCROLL:
			switch(LOWORD(wParam))
			{
				...
				case SB_LINEDOWN:
					...
					break;
				case SB_PAGEUP:
					iVscollPos -= cyClient / cyChar;
					break;
				case SB_PAGEDOWN:
					iVscollPos += cyClient / cyChar;
					break;
				case SB_THUMBPOSITION:
					iVscollPos = HIWORD(wParam);
					break;
				default:
					break;
			}
			...
			if(iVscollPos < 0)
				iVscollPos = 0;
			else if(iVscollPos > gpStrVec->iVectorSize - 1)
				iVscollPos = gpStrVec->iVectorSize - 1;

			if(iVscollPos != GetScrollPos(hWnd, SB_VERT))
			{
				SetScrollPos(hWnd, SB_VERT, iVscollPos, TRUE);
